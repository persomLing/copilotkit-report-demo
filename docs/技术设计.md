...
随着项目推进，规则会越来越多。不能把所有 `approvedSystemRules` 全量塞进 `useAgentContext`，否则会带来三个问题：
...
全量规则库
  ↓
retrieveRelevantRules(query, moduleId, topK)
  ↓
本轮 relevantRules Top K
  ↓
进入 useAgentContext / 工具返回值
```
...
- `scope`：全局、模块、用户。
- `moduleId`：规则适用模块。
- `priority`：业务优先级，越高越容易召回。
- `summary`：压缩后给模型看的短规则。
- `triggers`：轻量关键词检索。
- `examples/counterExamples`：帮助定义边界。
